...
async def send_message_group(group_number, text):
    session = DBSession()
...
async def handle_help(message):
    ...
    log_message(message)
    if not user_in:
        await bot.send_message(message.chat.id, 'Сначала необходимо зарегистрироваться!')
    else:
        user = session.query(User).filter_by(id=message.chat.id).one()
        if not user.type:
            await bot.send_message(message.chat.id, 'Ты зарегистрирован, как участник!\n'
                                                    ...
                                                    'По всем вопросам обращаться @f4r3t в телеграмм.\n'
            ...
            await bot.send_message(message.chat.id, 'Ты зарегистрирован, как организатор! Доступные тебе команды:\n'
                                                    ...
                                                    '/begin - старт квеста\n'
                                                    '/end - конец квеста\n'
                                                    '/stats - статистика по всем группам с cортировкой\n'
...
async def handler_user(message):
    session = DBSession()
    log_message(message)
    try:
        group = int(message.text.split()[1])
    except:
        await bot.send_message(message.chat.id, 'Неправильный формат!')
    else:
        x_in = check_group_in_bd(group, session)
        if not x_in:
            await bot.send_message(message.chat.id, 'Неправильный номер группы!')
        else:
            if len(list(session.query(User).filter_by(group=group).all())) >= MAX_GROUP_STUDENTS:
                await bot.send_message(message.chat.id, 'В выбранную группу уже нельзя зарегистрироваться!')
            else:
                ...
                if not user:
                    ...
                    await bot.send_message(message.chat.id, 'Ты уже зарегистрирован, второй раз это делать не нужно 😉')
    session.close()


# Информация о результатах группы
@bot.message_handler(commands=['info'])
async def handler_info(message):
    session = DBSession()
    log_message(message)
    user_in = check_user_in_bd(message, session)
    if not user_in:
        ...
        if check_group_in_bd(group_number, session):
            ...
            await bot.send_message(message.chat.id, 'Группа номер ' + str(group_number) +
                                   '\n🚩Текущая станция: ' + current_station +
                                   '\n💰Деньги: ' + str(group.money) +
                                   '\n✨Опыт: ' + str(group.experience))
        else:
            await bot.send_message(message.chat.id, 'У тебя нет группы(')
    session.close()


# Свободные станции
@bot.message_handler(commands=['free'])
async def handler_free(message):
    session = DBSession()
    log_message(message)
    ...
    if not is_started and not is_open:
        ...
        await bot.send_message(message.chat.id, 'Список свободных станций:\n\n' + answer)
    session.close()


# Взять станцию
@bot.message_handler(commands=['take'])
async def handler_take(message):
    session = DBSession()
    log_message(message)
    user_in = check_user_in_bd(message, session)
    if not user_in:
        ...
        if not is_started:
            ...
            group = session.query(Group).filter_by(id=user.group).one()
            if group.current_station != 0:
                await bot.send_message(message.chat.id, 'Вы ещё не закончили прохождение предыдущей станции!')
            else:
                try:
                    ...
                    if not station_in:
                        await bot.send_message(message.chat.id, 'Неправильный номер станции!')
                    else:
                        station = session.query(Station).filter_by(id=station_number).one()
                        if station.name in group.stations:
                            await bot.send_message(message.chat.id, 'Вы уже прошли эту станцию!')
                            session.close()
                            return
                            ...
                            session.commit()
                            await bot.send_message(message.chat.id,
                                                   'Ваша группа успешно зарегистрирована на станцию \"' +
                                                   station.name +
                                                   '\"! Организатор ждёт вас в следующем месте:\n📍' +
                                                   station.geo)
                            org = session.query(User).filter_by(station=station.id).one().id
                            await bot.send_message(org, 'Группа ' + str(
                                group.id) + ' была зарегистрирована на вашу станцию!')
                        else:
                            await bot.send_message(message.chat.id, 'Станция уже занята, попробуй найти другую')
...
async def handler_reg_org(message):
    session = DBSession()
    log_message(message)
    is_open = session.query(Settings).filter_by(name='registration').one().status
    if not is_open:
        await bot.send_message(message.chat.id, 'Регистрация организаторов закрыта!')
    else:
        try:
...
async def handler_station(message):
    global group
    session = DBSession()
    log_message(message)
    user_in = check_user_in_bd(message, session)
    if not user_in:
        await bot.send_message(message.chat.id, 'Организатор не зарегистрирован!')
    else:
        user = session.query(User).filter_by(id=message.chat.id).one()
        if user.type != 1 and user.type != 2:
            await bot.send_message(message.chat.id, 'Вы не организатор!')
        else:
            ...
            await bot.send_message(message.chat.id, str(station.id) + '. ' + station.name +
                                   '\nРасположение: ' + station.geo +
...
async def handler_reward(message):
    session = DBSession()
    log_message(message)
    user_in = check_user_in_bd(message, session)
    if not user_in:
        ...
        if user.type == 0:
            ...
            if not in_bd:
                await bot.send_message(message.chat.id, 'На вашей станции никого нет!')
            else:
                group = session.query(Group).filter_by(current_station=user.station).one()
                try:
                    ...
                    station.group = 0
                    session.commit()
                    await bot.send_message(message.chat.id, 'Группе ' + str(group.id) + ' успешно начисленны баллы!')
                    await send_message_group(group.id, 'Станция \"' + station.name + '\" успешно пройдена! 🎉'
        ...
        if user.type == 2 or user.type == 1:
            try:
                ...
                if amount <= 10:
                    ...
                    await bot.send_message(message.chat.id, 'Деньги успешно начилены.')
                    await send_message_group(group_number, 'Поздравляем! Твоей группе было начислено ' + str(
...
async def handler_mailing(message):
    session = DBSession()
    user_in = check_user_in_bd(message, session)
    if user_in:
        user = session.query(User).filter_by(id=message.chat.id).one()
...
async def handler_open(message):
    session = DBSession()
    log_message(message)
    user = session.query(User).filter_by(id=message.chat.id).one()
    if user.type == 2:
        ...
        session.commit()
        await bot.send_message(message.chat.id, 'Регистрация успешно открыта!')
    session.close()


# Закрыть регистрацию организаторов
@bot.message_handler(commands=['close'])
async def handler_close(message):
    session = DBSession()
    log_message(message)
    user = session.query(User).filter_by(id=message.chat.id).one()
    if user.type == 2:
        ...
        session.commit()
        await bot.send_message(message.chat.id, 'Регистрация успешно закрыта!!')
    session.close()


# Начать квест
@bot.message_handler(commands=['begin'])
async def handler_begin(message):
    session = DBSession()
    log_message(message)
    user = session.query(User).filter_by(id=message.chat.id).one()
    if user.type == 2:
        ...
        session.commit()
        await bot.send_message(message.chat.id, 'Квест успешно запущен!')
        users = session.query(User).all()
...
async def handler_end(message):
    ...
    if user.type == 2:
        ...
        session.commit()
        await bot.send_message(message.chat.id, 'Квест успешно закончен!')
...
async def handler_stats(message):
    session = DBSession()
    log_message(message)
    user = session.query(User).filter_by(id=message.chat.id).one()
    if user.type == 2:
...
async def destroy_button(message):
    session = DBSession()
    log_message(message)
    user = session.query(User).filter_by(id=message.chat.id).one()
    if user.type == 2:
        ...
        session.commit()
        return_classic_stats(session, roma)
        session.close()
...
async def handle_message(message):
    ...
    if message.text.lower() == 'привет':
        await bot.send_message(message.chat.id, random.choice(['Hello', 'Приветик', 'Пока', 'Ну, привет', 'Hi!']))
    elif message.text.lower() == 'как дела?':
        await bot.send_message(message.chat.id, random.choice(['Пока не родила',
                                                               'Великолепно',
        ...
        await bot.send_message(message.chat.id, random.choice(['чьь',
                                                               'Что?']))
    elif message.text.lower() == 'эй':
        await bot.send_message(message.chat.id, random.choice([f'Иди чаю попей',
                                                               'эээй-ээй-эй',
                                                               'Патимейкер',
                                                               'Уличный дэнсер! Фшик-фшик']))
    elif message.text.lower() == 'да':
        await bot.send_message(message.chat.id, random.choice(['Нет',
                                                               'Балда']))
    elif message.text.lower() == 'нет':
        await bot.send_message(message.chat.id, random.choice(['Ага, как же',
                                                               'Вообще-то да']))
    elif message.text.lower() == 'кто ты?':
        await bot.send_message(message.chat.id, random.choice(['Я искуственно выращенный '
                                                               'ИИ(мне так сказали)',
        ...
        await bot.send_message(message.chat.id, random.choice(['Я не знаю случайность это или нет, но '
                                                               'похоже ты нашел пасхалку, мой дорогой '
                                                               'друг. К сожалению на этой стадии я '
                                                               'ничего не могу пока тебе предложить.. '
                                                               'Хотя вот держи доп баллы от @f4r3t, только тсс, покажи это ей и получи свою награду'
                                                               ]))
    elif message.text.lower() == 'ты прелесть':
        await bot.send_message(message.chat.id, random.choice(['оаоаоаоаоаоаоаоааоаооаао',
                                                               'Спасибоооо))',
                                                               '-_-']))
    else:
        await bot.send_message(message.chat.id, random.choice(
